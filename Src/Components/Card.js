    ...
    return (
        <TouchableWithoutFeedback onPress={() => navigation.navigate('VideoPlayer', {videoId: item.id.videoId})}>
            <View style={{height:350, elevation:4, backgroundColor:colors.headerColor}}>
                <Image source={{uri:item.snippet.thumbnails.default.url}} style={{width:'100%', height:250}} />
                <View style={{marginTop:10, marginLeft:10, flexDirection:'row'}}>
                    ...
                    <View style={{flex:1, marginLeft:10, backgroundColor:colors.headerColor}}>
                        <Text numberOfLines={2} ellipsizeMode='tail' style={{fontSize:20, width:'95%', color:colors.iconColor}}>{item.snippet.channelTitle}</Text>
